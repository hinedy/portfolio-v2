...
export function diffAgainstContentPosts(
  ...
  for (const cp of contentPosts) {
    ...
    if (fp.title !== cp.title)
      issues.push(`title mismatch for ${cp.slug}: mdx="${fp.title}" vs content.ts="${cp.title}"`);
    if (fp.publishedAt !== cp.publishedAt)
      issues.push(
        `publishedAt mismatch for ${cp.slug}: mdx="${fp.publishedAt}" vs content.ts="${cp.publishedAt}"`,
      );
    if (fp.summary !== cp.summary)
      issues.push(
        `summary mismatch for ${cp.slug}: mdx="${fp.summary}" vs content.ts="${cp.summary}"`,
      );
  }
  const contentSlugs = new Set(contentPosts.map((p) => p.slug));
  for (const p of POSTS) {
    if (!contentSlugs.has(p.slug))
      issues.push(`.mdx "${p.slug}" has no entry in content.ts writing.posts.`);
  }
  return issues;
}
